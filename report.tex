...
\begin{table}[H]
    \centering
    \caption{Local vs Hugging Face Reload Comparison}
    \label{tab:results}
    \begin{tabular}{lcc}
        \toprule
        \textbf{Metric} & \textbf{Local} & \textbf{HF Reloaded (Docker)} \\
        \midrule
...
\subsection{Dataset Clarification}
Although the model is named \texttt{GoodReads BERT Classifier} on Hugging Face, it was fine-tuned using the \textbf{IMDb dataset} (movie reviews) as a proxy for sentiment analysis tasks. The naming convention was chosen to align with a broader project theme, but the underlying data for this specific assignment is IMDb.
